...
class Utils(object):
    ...
    @staticmethod
    def imageExists(results, _name, _tag):
        if not results:
            return False
        
        if not Utils.find_result(results, _name):
            return False
        
        if Utils.find_image_tag(results, _tag):
            return True
        
        return False

    @staticmethod
    def find_image_tag(results, _tag):
        '''Find an image tag by name'''
        rval = None
        for result in results:
            if 'spec' in result:
                for tag in result['spec']['tags']:
                    if 'name' in tag and tag['name'] == _tag:
                        rval = tag
                        break
        return rval
